...
for i in range(0,5):
    for noise in [0.25, 0.50]:
        try:
            embedding = numpy.load("all_embeddings_fold%i_noise%f.npy" % (i, noise))
            
            compact_embedding = embedding[::2,:] + embedding[1::2,:]
            numpy.save("all_embeddings_fold%i_noise%f_compact.npy" % (i, noise), compact_embedding)
            
            standardized_embedding = (compact_embedding -
                                    compact_embedding.mean(0)) / compact_embedding.std(0)
            numpy.save("all_embeddings_fold%i_noise%f_standardized.npy" % (i, noise), standardized_embedding)
